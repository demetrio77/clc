...
if [[ -z $DEVELOPMENT ]]; then
	...
	# Приведение прав в норму
	chown www-data:www-data -R /var/www/clc
 
	cd /var/www/clc
	[[ ! -f /var/www/clc/yii ]] && php init --env=Production --overwrite=y
	
	composer install --no-dev
else
	# В режиме разробочкика
	cd /var/www/clc
	# Если это первый запуск
	[[ ! -f /var/www/clc/yii ]] && php init --env=Development --overwrite=y
